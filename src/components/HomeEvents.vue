<template>
  <div class="container m-auto mt-10">
    <div class="px-8 py-4 bg-[#D1E0DB] rounded-xl">
      ...
      <el-row :gutter="30">
        <el-col v-for="event in events.slice(2, 8)" :key="event._id" :xs="24" :sm="12" :lg="8">
          <a :href="event.web" target="_blank">
            <el-card class="h-40 mb-5 md:hover:scale-105 hover:duration-500" :body-style="{ padding: '10px' }">
              <div class="flex flex-row h-full">
                <div class="mr-3">
                  <img :src="event.img" alt="" class="w-80 h-32 sm:w-44 md:w-80 lg:w-96 xl:w-80 rounded-lg mt-1" />
                </div>
                <div class="pb-9">
                  ...
                  <p class="line-clamp-4 text-sm h-fit mb-10">Địa điểm: {{ event.location }}</p>
                </div>
              </div>
            </el-card>
          </a>
...
onMounted(async () => {
  events.value = await getEvents(20, 0)
})
</script>
